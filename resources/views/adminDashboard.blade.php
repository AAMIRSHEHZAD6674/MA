...
@section('content')
    ...
    <form method="GET" action="{{ route('admin-dashboard') }}" class="mb-6">
        ...
        @if($user)
            <a href="{{ route('admin-dashboard') }}" class="ml-4 text-red-600 underline">Clear Filter</a>
        @endif
    </form>
...
@section('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const map = L.map('inspection-map').setView([30.3753, 69.3451], 6); // Centered on Pakistan
